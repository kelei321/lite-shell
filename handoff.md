...
当前状态：PR1～PR9 已完成并进入主线，`plan.md` 中的九阶段 SFTP 改造计划已经结束。双栏文件管理 PR1 已合并，用户已确认进入 PR2；PR2 当前在分支 `feat/sftp-path-toolbar-polish` 实现和验证。
...
当前任务：双栏文件管理 PR2“地址栏、工具栏和文件列表优化”正在分支 `feat/sftp-path-toolbar-polish` 实现和验证。用户已确认继续 PR2，不再停留在 PR1 本地验证门禁。
...
- PR2 完整 `npm run validate` 尚未在当前环境执行；等待 GitHub Actions 验证类型检查、前端测试、构建和 Rust 基线。
...
PR2 完成 code review 且 CI 通过后才能合并；合并后进入完整 SFTP 实机验收，不继续追加新的 SFTP 功能。
...
当前任务是双栏文件管理 PR2“地址栏、工具栏和文件列表优化”，分支为 feat/sftp-path-toolbar-polish。先核对 main...branch、PR 状态和 CI；如果 PR 尚未创建，确认目标文件后创建 PR；如果 CI 或 code review 有问题，只在同一分支做小范围修复。
...
- compare main...branch 后创建 PR，完成 code review，等待 CI 通过后再合并。
...
PR2 不改变远程读写、传输队列、冲突或删除语义；当前未执行任何真实服务器写操作。PR2 在 CI 和 code review 通过前不得合并。
