...
- 合并提交：`40502967e5874ed5e3b93e9c96862ab7c6ff3fbc`。
- 最终验证：GitHub Actions run `29402684994`，Frontend 类型检查/构建和 Rust 格式/测试/Clippy 全部通过。
- Code review 修复：根路径会主动懒加载；已展开但未加载的节点首次点击直接读取；刷新节点会保持展开；目录缩进改为 Vue 计算像素，避免依赖 WebView2 对 CSS 乘法表达式的支持。
- 后续补强：父节点缓存已加载但尚未包含当前目录时，选择路径会补齐父子关系，确保通过右侧列表、地址栏或历史进入的目录在左侧树中可见。
- 选中路径可见性补强 PR #13 已合并，合并提交为 `7490fc27979294d2f43c42950e834bc2cf249293`；GitHub Actions run `29419191950` 全部通过。
...
- 父节点已缓存后，通过地址栏或右侧列表进入服务器中新出现的目录，左树应显示并选中该目录。
- 分隔条拖动、宽度持久化和目录树折叠恢复正常。
- 中文、空格、深层目录和高延迟服务器。

用户已确认继续 PR2，PR1 本地验证门禁视为通过。
...
- 新增 `src/sftp/path-toolbar-shortcuts.ts`，支持 `Ctrl+L`、`Command+L` 和 `F6` 聚焦并全选路径输入框。
- 快捷键作用域排除终端和弹窗，终端内 `Ctrl+L` 继续发送给 SSH 会话，不改变现有终端行为。
- 新增 `src/sftp-polish.css`，在不重写 `App.vue` 的前提下覆盖现有 SFTP 地址栏、工具栏和文件列表样式。
- 面包屑分隔符改为 `>`，根节点只显示单个 `/`；路径输入聚焦和失败错误状态具有明确边框反馈。
- 导航、筛选设置和文件操作通过分隔与换行减少横向拥挤，窄窗口时工具栏限制高度并独立滚动。
- 文件列表增加“类型”列，目录显示“文件夹”，非目录显示“文件 / 链接”；列表使用独立滚动、稳定滚动条和 sticky 表头。
- `src/main.ts` 只负责加载覆盖样式和注册快捷键，没有修改 SFTP 业务流程。
- 未修改 `App.vue`、Rust 后端、远程目录枚举、上传下载、传输队列、冲突、删除或其他写入语义。
...
- 根路径和深层路径显示为 `/ > home > test`，不得出现 `//` 或重复分隔符。
- SFTP 文件区域按 `Ctrl+L`、`Command+L` 或 `F6` 后，路径输入框获得焦点并全选。
- 终端和弹窗内的快捷键不被地址栏接管。
- 路径跳转失败时当前有效目录保持不变、错误信息保留，再次编辑可恢复。
- 导航、筛选设置和文件操作在正常宽度清晰分组，在 1366、1180、960 等窄宽度下正常换行。
