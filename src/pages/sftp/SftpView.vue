<template>
  <section
    class="sftp-layout"
    @dragenter.prevent="showDropOverlay"
    @dragover.prevent="showDropOverlay"
    @dragleave.prevent="hideDropOverlay"
  ...
  >
    <header class="toolbar">
      ...
      <div class="status" :class="{ 'status--online': connectionId }">{{ statusText }}</div>
    </header>
    ...
    <section class="browser-card">
      ...
      <div class="sftp-toolbar-row">
        ...
        <div class="sftp-icon-actions" aria-label="SFTP 操作">
          <button class="sftp-icon-button" :disabled="!canBrowse || currentPath === '/'" title="上级" type="button" @click="goParent">↖</button>
          <button class="sftp-icon-button" :disabled="!canRunConnectedAction" title="上传文件" type="button" @click="uploadFile">↑</button>
          <button class="sftp-icon-button" :disabled="!canRunConnectedAction" title="上传文件夹" type="button" @click="uploadDirectory">⇈</button>
          <button class="sftp-icon-button" :disabled="!canDownloadSelected" title="下载" type="button" @click="downloadSelected">↓</button>
          <button class="sftp-icon-button" :disabled="!canRunConnectedAction" title="新建文件" type="button" @click="openCreateDialog('file')">□</button>
          <button class="sftp-icon-button" :disabled="!canRunConnectedAction" title="新建文件夹" type="button" @click="openCreateDialog('dir')">⊞</button>
          <button class="sftp-icon-button" :disabled="!canRename" title="重命名" type="button" @click="renameItem">✎</button>
          <button class="sftp-icon-button" :disabled="!hasSelection" title="复制" type="button" @click="copySelected">⧉</button>
          <button class="sftp-icon-button" :disabled="!hasSelection" title="剪切" type="button" @click="cutSelected">✂</button>
          <button class="sftp-icon-button" :disabled="!canPaste" title="粘贴" type="button" @click="pasteClipboard">▣</button>
          <button class="sftp-icon-button sftp-icon-button--danger" :disabled="!hasSelection" title="删除" type="button" @click="openDeleteDialog">⌫</button>
          <button class="sftp-icon-button" :disabled="autoConnecting || loading || actionLoading" title="刷新" type="button" @click="refresh">⟳</button>
          <button class="sftp-icon-button" :disabled="!canShowProperties" title="属性" type="button" @click="openPropertiesDialog('properties')">ⓘ</button>
          <button class="sftp-icon-button" :disabled="!canShowProperties" title="权限" type="button" @click="openPropertiesDialog('chmod')">🔒</button>
        </div>
      ...
      <div class="table-wrap" @click.self="clearSelection" @contextmenu.prevent="openContextMenu($event)">
        ...
        <table class="file-table">
          ...
          <tbody>
            <tr v-if="!connectionId"><td colspan="4" class="empty-cell">{{ statusNotice }}</td></tr>
            <tr v-else-if="loading"><td colspan="4" class="empty-cell">目录加载中...</td></tr>
            <tr v-else-if="files.length === 0"><td colspan="4" class="empty-cell">当前目录为空。</td></tr>
            <template v-else>
              <tr
                v-for="file in files"
                :key="file.path"
              ...
              >
                ...
                <td class="name-cell"><span class="file-icon">{{ file.isDir ? '📁' : '📄' }}</span><span>{{ file.name }}</span></td>
      ...
      <section class="transfer-panel" :class="{ 'transfer-panel--empty': transferTasks.length === 0 }">
        ...
        <div v-if="isTransferPanelExpanded && transferTasks.length" class="transfer-list transfer-list--table">
          <div class="transfer-row transfer-row--head"><span>类型</span><span>文件名</span><span>目标路径</span><span>大小</span><span>进度</span><span>状态</span><span>操作</span></div>
...
interface RemoteFileItem { name: string; path: string; isDir: boolean; size: number; }
...
interface TransferTask { id: string; type: TransferType; name: string; targetPath: string; transferredBytes: number; totalBytes: number; percent: number; status: TransferStatus; createdAt: number; }
interface ClipboardState { mode: ClipboardMode; sourceHostId: string; items: RemoteFileItem[]; }

type ActiveSessionSnapshot = { hostId: string; connectionId: string; currentPath: string };
...
let unlistenNativeFileDrop: (() => void) | undefined;

const activeHost = computed(() => workspaceStore.activeHost);
const cachedCredential = computed(() => activeHost.value ? workspaceStore.getCredential(activeHost.value.id) : undefined);
...
const activeHostLabel = computed(() => {
  ...
  return host ? `${host.username}@${host.host}` : '';
});
...
watch(() => ({ hostId: workspaceStore.activeHost?.id || '', credentialVersion: workspaceStore.credentialVersion }), () => {
  void syncSftpWithWorkspaceHost();
  void cleanupSessionsWithoutCredential();
}, { immediate: true });

watch(() => activeHost.value?.id || '', () => {
  clearSelection();
  clearClipboard();
  actionMessage.value = '';
});
...
onMounted(() => {
  ...
  void setupNativeFileDropListener();
...
async function setupNativeFileDropListener() {
  try {
    ...
    actionMessage.value = '拖拽上传监听初始化失败。';
  }
}

function updateTransferProgress(payload: SftpTransferProgressPayload) {
  transferTasks.value = transferTasks.value.map((task) => task.id === payload.transferId ? { ...task, transferredBytes: payload.transferredBytes, totalBytes: payload.totalBytes, percent: payload.percent, status: payload.status } : task);
}
...
async function connectSftpWithCredential(host: WorkspaceHost, credential: WorkspaceCredential) {
  ...
  try {
    const id = await invoke<string>('sftp_connect', { payload: { host: host.host, port: host.port, username: host.username, password: credential.password, privateKeyPath: null, passphrase: null } });
...
async function cleanupSessionsWithoutCredential() {
  for (const hostId of Object.keys(sessionsByHostId.value)) {
...
function isSelected(path: string) { return selectedPaths.value.has(path); }
function setSelectedPaths(paths: string[]) { selectedPaths.value = new Set(paths); }
function clearSelection() { setSelectedPaths([]); lastSelectedPath.value = ''; }
function selectAll() { setSelectedPaths(files.value.map((file) => file.path)); lastSelectedPath.value = files.value.at(-1)?.path || ''; }
function toggleSelectAll() { if (isAllSelected.value) clearSelection(); else selectAll(); }
...
function handleRowClick(event: MouseEvent, file: RemoteFileItem) {
  if (event.shiftKey && lastSelectedPath.value) { selectRange(file.path); return; }
  if (event.ctrlKey || event.metaKey) { toggleItem(file); return; }
...
function selectRange(path: string) {
  ...
  if (start < 0 || end < 0) { setSelectedPaths([path]); lastSelectedPath.value = path; return; }
...
function openContextMenu(event: MouseEvent, file?: RemoteFileItem) {
  event.preventDefault();
  if (file && !isSelected(file.path)) { setSelectedPaths([file.path]); lastSelectedPath.value = file.path; }
  contextMenu.visible = true;
  contextMenu.x = event.clientX;
  contextMenu.y = event.clientY;
}
function closeContextMenu() { contextMenu.visible = false; }
function runMenuAction(action: () => void | Promise<void>) { closeContextMenu(); void action(); }

function openItem(file: RemoteFileItem) {
  const hostId = activeHost.value?.id;
  if (!hostId || getCurrentSession()?.loading) return;
  if (file.isDir) { clearSelection(); void loadDir(file.path, hostId); }
  else { setSelectedPaths([file.path]); void downloadSelected(); }
}
function openSelected() { const item = selectedItems.value[0]; if (item) openItem(item); }
function goParent() {
  const hostId = activeHost.value?.id;
...
function refresh() {
  ...
  if (!session?.connectionId) { void syncSftpWithWorkspaceHost(); return; }
  void loadDir(session.currentPath || '/', host.id);
}

function copySelected() { setClipboard('copy'); }
function cutSelected() { setClipboard('cut'); }
...
function openCreateDialog(kind: CreateEntryKind) { createDialog.visible = true; createDialog.kind = kind; createDialog.name = ''; createDialog.error = ''; }
function closeCreateDialog() { createDialog.visible = false; createDialog.error = ''; }
async function confirmCreateEntry() {
  const normalizedName = normalizeRemoteName(createDialog.name);
  if (!normalizedName) { createDialog.error = '名称不能为空，且不能包含 / 或 \\'; return; }
  const snapshot = await ensureActiveSession();
  if (!snapshot) return;
...
async function uploadFile() {
  ...
  for (const path of paths) await uploadOneFile(snapshot, path);
}
...
async function uploadDroppedPaths(paths: string[]) {
  ...
  actionStatusText.value = `正在拖拽上传 ${paths.length} 项...`;
  actionMessage.value = '';
  try {
    for (const path of paths) await uploadOneDroppedPath(snapshot, path);
    await loadDir(snapshot.currentPath, snapshot.hostId);
    actionMessage.value = `拖拽上传完成：${paths.length} 项。`;
  } finally {
    actionLoading.value = false;
    actionStatusText.value = '';
  }
}
async function uploadOneFile(snapshot: ActiveSessionSnapshot, localPath: string) {
...
async function uploadOneDirectory(snapshot: ActiveSessionSnapshot, localPath: string) {
...
async function uploadOneDroppedPath(snapshot: ActiveSessionSnapshot, localPath: string) {
  const name = getLocalFileName(localPath);
  if (!name) return;
  const remotePath = joinRemotePath(snapshot.currentPath, name);
  const transferId = createTransferTask('upload', name, snapshot.currentPath);
  try {
...
async function downloadSelected() {
  ...
  if (items.length === 1 && !items[0].isDir) { await downloadOneFile(items[0]); return; }
  const targetDir = await open({ directory: true, multiple: false });
  if (!targetDir || Array.isArray(targetDir)) return;
  for (const item of items) {
    if (item.isDir) await downloadOneDirectory(item, targetDir);
    else await downloadOneFile(item, joinLocalPath(targetDir, item.name));
  }
}
...
function openDeleteDialog() { if (selectedItems.value.length) deleteDialog.visible = true; }
function closeDeleteDialog() { deleteDialog.visible = false; }
...
async function openPropertiesDialog(mode: 'properties' | 'chmod') {
  ...
  try {
    ...
    if (stat.permissions && stat.permissions !== '-') chmodDialog.mode = stat.permissions.slice(-4).replace(/^0+/, '') || stat.permissions;
  } catch {
    if (!propertiesDialog.visible || propertiesDialog.item?.path !== item.path) return;
    propertiesDialog.error = '属性读取失败，请检查远程权限。';
  } finally {
    if (propertiesDialog.item?.path === item.path) propertiesDialog.loading = false;
  }
}
function closePropertiesDialog() { propertiesDialog.visible = false; propertiesDialog.item = null; propertiesDialog.stat = null; propertiesDialog.loading = false; propertiesDialog.error = ''; }
async function savePermissions() {
  const item = propertiesDialog.item;
  const snapshot = await ensureActiveSession();
  if (!item || !snapshot) return;
  if (!/^[0-7]{3,4}$/.test(chmodDialog.mode)) { actionMessage.value = '权限格式不正确，请输入 755 这类八进制权限。'; return; }
...
async function ensureActiveSession() {
  ...
  if (!snapshot) { await syncSftpWithWorkspaceHost(); snapshot = getActiveSessionSnapshot(); }
  if (!snapshot) actionMessage.value = activeHost.value ? 'SFTP 尚未连接。' : '请先连接 SSH 主机。';
  return snapshot;
}
function getActiveSessionSnapshot(): ActiveSessionSnapshot | undefined {
  const host = activeHost.value;
...
async function runSftpAction(options: { snapshot: ActiveSessionSnapshot; loadingText: string; successText: string; failureText: string; transferId?: string; action: () => Promise<unknown> }) {
  actionLoading.value = true;
...
function handleDropUpload() { isDropActive.value = false; }
function joinRemotePath(basePath: string, name: string) { if (basePath === '/') return `/${name}`; return `${basePath.replace(/\/+$/, '')}/${name}`; }
function joinLocalPath(basePath: string, name: string) { return `${basePath.replace(/[\\/]+$/, '')}/${name}`; }
function normalizeRemoteName(value: string | null) { const name = value?.trim() || ''; if (!name || name === '.' || name === '..' || name.includes('/') || name.includes('\\')) return ''; return name; }
function getLocalFileName(path: string) { return path.split(/[\\/]/).pop() || ''; }
function getParentPath(path: string) { const normalized = path.replace(/\/+$/, ''); const index = normalized.lastIndexOf('/'); if (index <= 0) return '/'; return normalized.slice(0, index); }
function formatPercent(value: number | undefined) { if (value === undefined || Number.isNaN(value)) return '-'; return `${Math.min(100, Math.max(0, value)).toFixed(1)}%`; }
function progressWidth(value: number | undefined) { if (value === undefined || Number.isNaN(value)) return '0%'; return `${Math.min(100, Math.max(0, value))}%`; }
function formatModifiedAt(value: number | undefined) { if (!value) return '-'; return new Date(value * 1000).toLocaleString(); }
function formatOwner(stat: RemoteFileStat) { const uid = stat.uid === undefined ? '-' : stat.uid; const gid = stat.gid === undefined ? '-' : stat.gid; return `${uid}:${gid}`; }
function formatSize(size: number) { if (size < 1024) return `${size} B`; if (size < 1024 * 1024) return `${(size / 1024).toFixed(1)} KB`; if (size < 1024 * 1024 * 1024) return `${(size / 1024 / 1024).toFixed(1)} MB`; return `${(size / 1024 / 1024 / 1024).toFixed(1)} GB`; }
</script>
...
.browser-card { position: relative; display: flex; min-width: 0; min-height: 0; flex: 1; flex-direction: column; overflow: hidden; border: 1px solid var(--ls-border); border-radius: 12px; background: var(--ls-panel); }
...
.sftp-path-main { min-width: 0; width: 100%; max-width: 420px; height: 28px; overflow: hidden; border: 1px solid var(--ls-border); border-radius: 8px; background: var(--ls-panel); color: var(--ls-text); font-size: 12px; line-height: 26px; padding: 0 10px; text-overflow: ellipsis; white-space: nowrap; }
.sftp-icon-actions { display: inline-flex; align-items: center; gap: 5px; }
.sftp-icon-button { display: inline-grid; width: 28px; height: 28px; place-items: center; overflow: hidden; border: 1px solid var(--ls-border-strong); border-radius: 8px; background: linear-gradient(180deg, var(--ls-panel), var(--ls-panel-strong)); color: var(--ls-text); cursor: pointer; font-size: 15px; font-weight: 700; padding: 0; }
